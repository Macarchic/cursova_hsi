...
from improved_paper.preprocessing import pca_class_outlier_mask
...
def _fps_indices(vectors: np.ndarray, k: int, outlier_std: float = 2.5) -> np.ndarray:
    ...
    1. Remove outliers: discard samples whose L2 distance to the class centroid
       exceeds mean_dist + outlier_std * std_dist.
    2. Start FPS from the inlier closest to the centroid.
...
def create_split(
    ...
    use_fps=True → training samples chosen via Farthest Point Sampling on PCA
    spectral vectors (with outlier pre-filtering). Otherwise a random split.
