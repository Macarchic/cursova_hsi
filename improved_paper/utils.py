...
# Module-level device, used by the copied SCMT layers (layers_scmt.py) for the
# runtime-created linears and attention masks in the original `former`.
device = detect_accelerator()[1]
...
def get_run_dir(name: str, results_root: str = 'results') -> Path:
    base = Path(results_root)
    v = 1
    while (base / f'{name}_v{v}').exists():
        v += 1
    run_dir = base / f'{name}_v{v}'
