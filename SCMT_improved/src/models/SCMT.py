...
class MultiRingCenterAttention(nn.Module):
    ...
    Центральний піксель = Query; сусіди по кільцях = Key/Value:
      Ring1 = 8 безпосередніх сусідів, Ring2 = решта токенів.
    Два виходи зливаються через learned merge + residual від центрального токена.
    Портовано з improved/model.py і адаптовано під сітку токенів SCMT (grid_side),
    яка після conv2d МЕНША за patch_size (padding=1, kernal=9).
    """

    def __init__(self, dim, grid_side, num_heads):
        super().__init__()
        ...
        self.register_buffer('ring1_idx', torch.tensor(ring1, dtype=torch.long))
        self.register_buffer('ring2_idx', torch.tensor(ring2, dtype=torch.long))

        self.norm = nn.LayerNorm(dim)
        self.q_proj = nn.Linear(dim, dim, bias=False)
        self.kv_proj = nn.Linear(dim, 2 * dim, bias=False)
        self.merge = nn.Linear(2 * dim, dim, bias=False)  # злиття двох кілець
        self.proj = nn.Linear(dim, dim, bias=False)
        # Ручний identity-init merge не робимо: Init(net) у трейнері
        # все одно перезапише всі nn.Linear ортогонально.

    def _ring_attn(self, q, k, v, idx):
        """Attention від центрального Q до підмножини K/V токенів (idx)."""
        B = q.shape[0]
        n = idx.shape[0]
    ...
    def forward(self, x):
        ...
        k, v = self.kv_proj(xn).chunk(2, dim=-1)                       # (B,T,D) кожен
...
class SCMT(nn.Module):
    def __init__(self, params):
        ...
        #   grid_side = patch_size + 2*padding - kernal + 1
        patch_size = data_params.get("patch_size", 15)
        grid_side = patch_size + 2 * padding - kernal + 1
        center_heads = net_params.get("center_heads", 8)  # має ділити dim (64)
        use_center_attention = net_params.get("center_attention", True)
        if use_center_attention:
    ...
    def encoder_block(self, x):
        ...
        if self.center_attention is not None:
            logit_x = self.center_attention(x_pixel)
        else:
